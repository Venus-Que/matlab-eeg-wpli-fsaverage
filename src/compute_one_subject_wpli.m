...
%
% 输出参数：
%   mat_path - 包含四频段59x59或64x64矩阵及质量控制参数的MAT文件。
%   png_path - 矩阵和二维网络汇总图。
%
...
band_ranges_hz = [1 4; 4 8; 8 13; 13 30];
band_titles = {'delta 1-4 Hz', 'theta 4-8 Hz', ...
...
fs = hdr.Fs;
assert(fs / 2 >= band_ranges_hz(end, 2), ...
    '采样率%.1f Hz的Nyquist频率低于beta上限30 Hz。', fs);
...
cfg.output = 'fourier';
cfg.taper = 'dpss';
cfg.foilim = [band_ranges_hz(1, 1), band_ranges_hz(end, 2)];
...
band_matrices = struct;
band_frequency_bins_hz = struct;
for band_idx = 1:numel(band_names)
    frequency_mask = select_band_frequencies(conn.freq, ...
        band_ranges_hz(band_idx, :), band_idx == numel(band_names));
    assert(any(frequency_mask), '频率范围中缺少%s频点。', ...
        band_names{band_idx});
    band_frequency_bins_hz.(band_names{band_idx}) = conn.freq(frequency_mask);
    current_matrix = mean(full_matrix(:, :, frequency_mask), 3, 'omitnan');
    band_matrices.(band_names{band_idx}) = force_symmetric(current_matrix);
end

% 保留独立变量名，兼容旧版下游脚本直接读取theta_matrix/alpha_matrix。
delta_matrix = band_matrices.delta;
theta_matrix = band_matrices.theta;
alpha_matrix = band_matrices.alpha;
beta_matrix = band_matrices.beta;
...
band_global_means = struct;
for band_idx = 1:numel(band_names)
    current_matrix = band_matrices.(band_names{band_idx});
    band_global_means.(band_names{band_idx}) = ...
        mean(current_matrix(upper_mask), 'omitnan');
end
delta_global_mean = band_global_means.delta;
theta_global_mean = band_global_means.theta;
alpha_global_mean = band_global_means.alpha;
beta_global_mean = band_global_means.beta;

%% 14. 绘制矩阵和脑网络
fig = figure('Color', 'w', 'Position', [40 60 1900 1050]);
plot_layout = tiledlayout(fig, 2, numel(band_names), ...
    'TileSpacing', 'compact', 'Padding', 'compact');

for band_idx = 1:numel(band_names)
    current_name = band_names{band_idx};
    current_matrix = band_matrices.(current_name);

    matrix_ax = nexttile(plot_layout, band_idx);
    imagesc(matrix_ax, current_matrix);
    axis(matrix_ax, 'image');
    colorbar(matrix_ax);
    caxis(matrix_ax, [-0.5 1]);
    title(matrix_ax, sprintf('%s\n全局平均=%.4f', ...
        band_titles{band_idx}, band_global_means.(current_name)));
    xlabel(matrix_ax, '电极');
    ylabel(matrix_ax, '电极');
    set_matrix_ticks(matrix_ax, data.label);

    network_ax = nexttile(plot_layout, numel(band_names) + band_idx);
    draw_network(network_ax, current_matrix, xy, n_edges_to_plot);
    title(network_ax, sprintf('%s最强%d条正连接', ...
        band_titles{band_idx}, n_edges_to_plot));
end
...
params.bands = struct('delta', [1 4], 'theta', [4 8], ...
    'alpha', [8 13], 'beta', [13 30]);
params.band_names = band_names;
params.band_ranges_hz = band_ranges_hz;
params.band_boundary_rule = ...
    'Lower bound included; upper bound excluded except beta includes 30 Hz.';
params.frequency_limits_hz = [band_ranges_hz(1, 1), band_ranges_hz(end, 2)];
params.tapsmofrq_hz = 1.0;
...
save(mat_path, 'delta_matrix', 'theta_matrix', 'alpha_matrix', 'beta_matrix', ...
    'delta_global_mean', 'theta_global_mean', ...
    'alpha_global_mean', 'beta_global_mean', ...
    'band_matrices', 'band_global_means', 'band_frequency_bins_hz', ...
    'xy', 'params', ...
...
function mask = select_band_frequencies(frequencies, limits_hz, include_upper)
    if include_upper
        mask = frequencies >= limits_hz(1) & frequencies <= limits_hz(2);
    else
        mask = frequencies >= limits_hz(1) & frequencies < limits_hz(2);
    end
end
